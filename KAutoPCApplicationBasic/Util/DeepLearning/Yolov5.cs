...
namespace KAutoPCApplicationBasic.Util
{
        ...
        : PredictorBase, IPredictor
    {
        ...
        private List<Prediction> ParseDetect(DenseTensor<float> output, Mat image)
        {
        ...
        private List<Prediction> ParseSigmoid(DenseTensor<float>[] output, Mat image)
        {
            var result = new ConcurrentBag<Prediction>();
        ...
        protected List<Prediction> ParseOutput(DenseTensor<float>[] output, Mat image)
        {
            return UseDetect ? ParseDetect(output[0], image) : ParseSigmoid(output, image);

        }

        public override Prediction[] Predict(Mat image)
        {
            return Suppress(
             ParseOutput(
