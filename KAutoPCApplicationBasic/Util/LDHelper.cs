...
namespace KAutoPCApplicationBasic.Util
{
    ...
    {
        public static string pathLD = "C:\\LDPlayer\\LDPlayer9\\ldconsole.exe";
        ...
        public void UnInstallApp(string param, string NameOrId, string Package_Name) => this.ExecuteLD(string.Format("uninstallapp {0} --packagename {1}", (object)NameOrId, (object)Package_Name));
        ...
        public List<Info_Devices2> GetDevices3_Running()
        {
            try
            {
                int index = 0;
                // New List Info Device 2
                List<Info_Devices2> infoDevicesList = new List<Info_Devices2>();
                // Get device dang open
                ...
                foreach (string str2 in str1.Split(chArray))
                {
                    Info_Devices2 infoDevices = new Info_Devices2();
                    string[] strArray = str2.Trim().Split(',');
                    infoDevices.index = int.Parse(strArray[0]);
                    infoDevices.name = strArray[1];
                    infoDevices.tophw = Int32.Parse(strArray[2].ToString());
                    infoDevices.bindhw = Int32.Parse(strArray[3].ToString());
                    if (devicesRunning.Contains(infoDevices.name))
                    {
                        infoDevices.adb_id = "None";
                        infoDevicesList.Add(infoDevices);
                    }
                }
                return infoDevicesList;
        ...
        public static string GetIMEI(string deviceindex)
        {
            try
            {
                ...
                {
                    WorkingDirectory = @"C:\LDPlayer\LDPlayer9",
                    FileName = "cmd.exe",
                    CreateNoWindow = true,
                ...
                var result1 = process.StandardOutput.ReadToEnd().Trim();
                if (result1.Contains("\r\n"))
                {
                    var result = result1.Split(new string[] { "\r\n" }, System.StringSplitOptions.RemoveEmptyEntries);
                    if (result.Where(x=> x.Length == 15 || x.Length == 16 || x.Length == 17).Any()) {
                        return result.Where(x => x.Length == 15||x.Length==16||x.Length==17).FirstOrDefault();
                    }
                    else
                    {
                        return "";
